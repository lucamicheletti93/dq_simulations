...
for tf in range(1, NTIMEFRAMES + 1):
   ...
   includeQED = False # (COLTYPE == 'PbPb' or (doembedding and COLTYPEBKG == "PbPb")) or (args.with_qed == True)
   ...
   PreCollContextTask['cmd']='${O2_ROOT}/bin/o2-steer-colcontexttool -i bkg,' + str(INTRATE) + ',' + str(args.nb) + ':' + str(args.nb) + ' --show-context ' + ' --timeframeID ' + str(tf-1 + int(args.production_offset)*NTIMEFRAMES) + ' --orbitsPerTF ' + str(orbitsPerTF) + ' --orbits ' + str(orbitsPerTF) + ' --seed ' + str(TFSEED) + ' --noEmptyTF --first-orbit ' + str(args.first_orbit)
   ...
   if doembedding:
      ContextTask['cmd'] += ';ln -nfs ../bkg_Kine.root .;${O2_ROOT}/bin/o2-steer-colcontexttool -i bkg,' + str(INTRATE) + ',' + str(args.nb) + ':' + str(args.nb) + ' ' + signalprefix + ',' + args.embeddPattern + ' --show-context ' + ' --timeframeID ' + str(tf-1 + int(args.production_offset)*NTIMEFRAMES) + ' --orbitsPerTF ' + str(orbitsPerTF) + ' --use-existing-kine'
   ...
   MCHRECOtask['cmd'] += '${O2_ROOT}/bin/o2-mch-reco-workflow ' + getDPL_global_options() + putConfigValues({"MCHStatusMap.useBadChannels" : 0, "MCHStatusMap.useRejectList" : 1, "MCHStatusMap.useHV": 1, "MCHDigitFilter.statusMask" : 3})
   MCHRECOtask['cmd'] += ('',' --disable-mc')[args.no_mc_labels]
   # MCHRECOtask['cmd'] += " --enable-clusters-root-output"
   # MCHRECOtask['cmd'] += " | o2-qc --config json://${SIMDIR}/mch-qc.json --local-batch QC.root -b"
   ...
   if includeFullQC or includeLocalQC:
     ...
     if isActive('MCH'):
          addQCPerTF(taskName='MCHTaskQC',
                  needs=[MCHRECOtask['name']],
                  readerCommand='o2-mch-tracks-reader-workflow',
                  configFilePath='json://${SIMDIR}/mch-qc.json')
