...
def main():
    ...
    parser.add_argument("-ns", type=int, help="number of signal events per timeframe", default=1)
    parser.add_argument("-nb", type=int, help="number of background events per timeframe", default=0)
    parser.add_argument("-do-embedding", type=bool, help="do embedding", default=False)
    ...
    forwardargs += " -ns " + str(args.ns) + " -tf " + str(args.tf) + " --sor " + str(run_start) + " --timestamp " + str(timestamp) + " --production-offset " + str(prod_offset) + " -run " + str(args.run_number) + " --run-anchored --first-orbit "       \
    ...
    if args.do_embedding:
      sig_rate = rate * args.ns / args.nb
      forwardargs += f' -nb {args.nb} --embedding --embeddPattern {sig_rate},{args.ns}:{args.ns * args.tf}'

    print ("forward args ", forwardargs)
    cmd = "./o2dpg_sim_workflow.py " + forwardargs

    print("********* RUNNING ARGUMENTS ***********")
    print(f'RUN {args.run_number}')
    print(f'SPLITID {args.split_id}')
    print(f'NTIMEFRAMES {args.tf}')
    print(f'NSIGPTF {args.ns}')
    print(f'SIGINTRATE {sig_rate}')
    if args.do_embedding:
      print(f'BKGINTRATE {rate}')
    print("********* DONE ************************")
